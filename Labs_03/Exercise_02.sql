...
SELECT (SELECT CompanyName FROM Shippers WHERE ShipperID = ShipVia) AS 'Shipper', COUNT(*) AS 'Count'
FROM Orders
GROUP BY ShipVia

-- 5. Który z spedytorów był najaktywniejszy w 1997 roku
SELECT TOP 1 WITH TIES (SELECT CompanyName FROM Shippers WHERE ShipperID = ShipVia) AS 'Shipper', COUNT(*) AS 'Count'
FROM Orders
