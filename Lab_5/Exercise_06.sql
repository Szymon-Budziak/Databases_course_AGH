-- 1. Napisz polecenie które zwraca imię i nazwisko (jako pojedynczą kolumnę – name), oraz informacje o adresie:
...
SELECT m.member_no,
       ...
       m.lastname
FROM member AS m
         INNER JOIN adult a ON m.member_no = a.member_no AND a.state = 'AZ'
...
FROM member AS m
         INNER JOIN adult a ON m.member_no = a.member_no
WHERE a.state = 'AZ'
  AND (SELECT COUNT(*) FROM juvenile AS j WHERE j.adult_member_no = a.member_no) > 2
UNION
SELECT m2.member_no,
       m2.firstname,
       m2.lastname
FROM member AS m2
         INNER JOIN adult a2 ON m2.member_no = a2.member_no
WHERE a2.state = 'CA'
  AND (SELECT COUNT(*) FROM juvenile AS j WHERE j.adult_member_no = a2.member_no) > 3