...
SELECT S.CompanyName, COUNT(*) AS Number_of_orders
FROM Shippers AS S
...
-- tego przewoźnika
SELECT TOP 1 S.CompanyName, COUNT(*) AS Number_of_orders
FROM Shippers AS S
         INNER JOIN Orders O ON S.ShipperID = O.ShipVia
WHERE YEAR(ShippedDate) = 1997
GROUP BY S.CompanyName
...
SELECT TOP 1 E.FirstName, E.LastName, COUNT(*) AS Number_of_orders
FROM Employees AS E
         INNER JOIN Orders O ON E.EmployeeID = O.EmployeeID
...
SELECT TOP 1 E.FirstName, E.LastName, ROUND(SUM(UnitPrice * Quantity * (1 - Discount)), 2) AS total_value
FROM Employees AS E
         INNER JOIN Orders O ON E.EmployeeID = O.EmployeeID
         INNER JOIN [Order Details] OD ON O.OrderID = OD.OrderID
